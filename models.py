...
class ResNet(nn.Module):
    def __init__(self, block, nblocks, num_classes=[10]):
        ...
        size_linear = 512
        # 1st Exit
        self.end_bns1 = nn.ModuleList([nn.Sequential(nn.BatchNorm2d(int(64*factor)),nn.ReLU(True)) for i in range(nb_tasks)])
        self.linears1_inter = nn.ModuleList([conv1x1(int(64*factor),int(32*factor)) for i in range(nb_tasks)])         
        self.avgpool1 = nn.AdaptiveAvgPool2d(1)
        self.linears1 = nn.ModuleList([nn.Linear(int(32*factor), num_classes[i]) for i in range(nb_tasks)])         
        
        self.layer2 = self._make_layer(blocks[1], int(128*factor), nblocks[1], stride=2, nb_tasks=nb_tasks)
        
        # 2nd Exit
        self.end_bns2 = nn.ModuleList([nn.Sequential(nn.BatchNorm2d(int(128*factor)),nn.ReLU(True)) for i in range(nb_tasks)])
        self.linears2_inter = nn.ModuleList([conv1x1(int(128*factor),int(64*factor))  for i in range(nb_tasks)])
        self.avgpool2 = nn.AdaptiveAvgPool2d(1)
        self.linears2 = nn.ModuleList([nn.Linear(int(64*factor), num_classes[i]) for i in range(nb_tasks)])         
        
        self.layer3 = self._make_layer(blocks[2], int(256*factor), nblocks[2], stride=2, nb_tasks=nb_tasks)
        
        # 3rd Exit
        self.end_bns3 = nn.ModuleList([nn.Sequential(nn.BatchNorm2d(int(256*factor)),nn.ReLU(True)) for i in range(nb_tasks)])
        self.avgpool3 = nn.AdaptiveAvgPool2d(1)
        #self.linears3_inter = nn.ModuleList([nn.Linear(int(256*factor),size_linear) for i in range(nb_tasks)])         
        self.linears3 = nn.ModuleList([nn.Linear(int(256*factor), num_classes[i]) for i in range(nb_tasks)])         
    ...
    def forward(self, x):
        ...
        x1 = self.end_bns1[task](x)

        x1 = self.linears1_inter[task](x1)
        x1 = self.avgpool1(x1)
        x1 = x1.view(x1.size(0), -1)
        x1 = self.linears1[task](x1)
        
        x = self.layer2(x)
        
        x2 = self.end_bns2[task](x)

        x2 = self.linears2_inter[task](x2)
        x2 = self.avgpool2(x2)
        x2 = x2.view(x2.size(0), -1)
        x2 = self.linears2[task](x2)

        
        x = self.layer3(x)
        
        x3 = self.end_bns3[task](x)
        x3 = self.avgpool3(x3)
        x3 = x3.view(x.size(0), -1)
        #x3 = self.linears3_inter[task](x3)
        x3 = self.linears3[task](x3)
        #print(x3[0,:].sum(), x3.shape)
        return x1, x2, x3
