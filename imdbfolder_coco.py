...
class ImageFolder_online(data.Dataset):
...
def prepare_data_loaders(dataset_names, data_dir, imdb_dir, shuffle_train=True, index=None):
    ...
    return train_loaders, val_loaders, num_classes, min_lab


def prepare_data_loaders_online(dataset_names, data_dir, imdb_dir, shuffle_train=False, index=None):
    val_loaders = []
    ...
    for i in range(len(dataset_names)):
        ...
        valloader = torch.utils.data.DataLoader(ImageFolder_online(data_dir, transform_test, None, None, None, imgnames_val), batch_size=100, shuffle=False, num_workers=4, pin_memory=True)
        val_loaders.append(valloader) 
    
    return val_loaders, timgnames_id
